...
dim shared showpage as integer = 0
...
sub gfx_screenres
        if windowed = 0 then
                screenres screenmodex, screenmodey, depth, 2, GFX_FULLSCREEN 'or GFX_HIGH_PRIORITY
        else
                screenres screenmodex, screenmodey, depth, 2, GFX_WINDOWED 'or GFX_HIGH_PRIORITY
        end If
        screenset 1, 0
	showpage = 0
end sub
...
d2 -= TIMER
	'screensync
     '   screenunlock
        'screensync
        d = TIMER
	showpage = showpage xor 1
'	screenset showpage xor 1, showpage
        flip
        d -= TIMER
        'debug "blit = " & -d2 & " flip = " & -d
