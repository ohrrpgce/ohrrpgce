...
Function CompareNodes(byval nod1 as nodeptr, byval nod2 as nodeptr) as integer
	...
	select case NodeType(nod1)
		...
		case rltString
			...
			'Oh you're not in the clear yet! Might have contained null characters
			if GetZStringSize(nod1) <> GetZStringSize(nod2) orelse _
			   memcmp(GetZString(nod1), GetZString(nod2), GetZStringSize(nod1)) then
				debug "Value of node " & GetNodePath(nod1) & " differ! Binary blobs length " & GetZStringSize(nod1) & " vs length " & GetZStringSize(nod2)
				return 1
			end if
	end select
	...
	for i as integer = 0 to numkids - 1
		if CompareNodes(nod1, nod2) then
