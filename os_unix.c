...
void crash_handler(int signum, siginfo_t *si, void *ucontext) {
	save_backtrace(false);
	fatal_error("Signal %d errno %d code %d addr %p", si->si_signo, si->si_errno, si->si_code, si->si_addr);
	//debuginfo("Sig %d errno %d code %d addr %p", si->si_signo, si->si_errno, si->si_code, si->si_addr);
}
