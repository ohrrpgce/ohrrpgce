...
struct signal_name {int signo; const char *name;};
const struct signal_name signal_names[] = {
	{SIGABRT, "SIGABRT"},
	{SIGFPE,  "SIGFPE"},
	{SIGILL,  "SIGILL"},
	{SIGSEGV, "SIGSEGV"},
...
void fatal_signal_handler(int signum, siginfo_t *si, void *ucontext) {
	...
	const char *signame = "";
	const struct signal_name *namepair = signal_names;
	for (; namepair->signo; namepair++) {
		if (namepair->signo == si->si_signo)
			signame = namepair->name;
	}
	...
	ADDTEXT("Aborting due to signal %d %s code %d address %p", si->si_signo, signame, si->si_code, si->si_addr);
...
// -we call save_backtrace() immediately, reducing chance of a double-fault before it can run
...
boolint setup_exception_handler() {
	...
	sigaction(SIGQUIT, &sa, NULL);  // Generated by C-\ on terminal
	return YES;
}
