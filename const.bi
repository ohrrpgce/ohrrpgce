...
' 7 - ichorescent switched to extra data vectors in slices
...
CONST genMaxHero = 35           'max hero definition number in heroes.reld (or .DT0)
...
CONST genJoy = 60               'Used to be whether the joystick is enabled (not respected anywhere anymore)
...
CONST genNumBackdrops = 100     'number of backdrops
...
CONST maxScriptCmdID = 768  'Max ID number of any supported script command (checked when saving or loading game)
