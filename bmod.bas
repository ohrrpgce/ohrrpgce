...
DECLARE SUB check_death(deadguy AS INTEGER, BYVAL killing_attack AS INTEGER, BYREF bat AS BattleState, BYREF rew AS RewardsState, bstat() AS BattleStats, bslot() AS BattleSprite, es(), formdata(), p(), bits(), ebits(), targmem(), BYREF tptr AS INTEGER, BYREF ptarg AS INTEGER, ltarg())
...
DIM fctr(24), harm$(11), hc(23), hx(11), hy(11), conlmp(11), bits(11, 4), iuse(15), icons(11), ebits(40), ltarg(11), lifemeter(3), revengeharm(11), repeatharm(11), targmem(23), prtimer(11,1), spelmask(1)
...
autotarget bat.enemy_turn, atktemp(), bslot(), bstat(), targmem()
...
DO: 'INTERPRET THE ANIMATION SCRIPT
 ...
 SELECT CASE act
  ...
  CASE 10 'inflict(targ, target_count)
   ...
   IF inflict(bat.acting, targ, bstat(), bslot(), harm$(), hc(), hx(), hy(), atk(), tcount, bits(), targmem(), revengeharm(), repeatharm()) THEN
    '--attack succeeded
   ...
   IF bstat(targ).cur.hp = 0 AND o < 8 AND anim > -1 THEN'
    ...
    autotarget bat.acting, atk(), bslot(), bstat(), targmem()
   END IF
  CASE 11 'setz(who,z)
   ww = popw
   bslot(ww).z = popw
 ...
 IF atk(12) > 0 AND INT(RND * 100) < atk(13) AND bstat(bat.acting).cur.hp > 0 AND (bslot(bat.acting).attack_succeeded <> 0 AND readbit(atk(),65,7) OR readbit(atk(),65,7) = 0)THEN
  ...
  IF o < 8 THEN
   IF buffer(4) <> atk(4) OR buffer(3) <> atk(3) THEN
    ...
    autotarget bat.acting, buffer(), bslot(), bstat(), targmem()
   END IF
  END IF
 END IF
END IF
...
get_valid_targs bat.targ.mask(), bat.hero_turn, buffer(), bslot(), bstat(), targmem()
...
NEXT
FOR deadguy = 0 TO 11
 check_death deadguy, 0, bat, rew, bstat(), bslot(), es(), formdata(), p(), bits(), ebits(), targmem(), tptr, ptarg, ltarg()
NEXT
deadguycount = 0
FOR deadguy = 4 TO 11
...
'autotarget
IF ptarg = 3 THEN
 autotarget bat.hero_turn, buffer(), bslot(), bstat(), targmem()
 ctr(bat.hero_turn) = 0
...
SUB check_death(deadguy AS INTEGER, BYVAL killing_attack AS INTEGER, BYREF bat AS BattleState, BYREF rew AS RewardsState, bstat() AS BattleStats, bslot() AS BattleSprite, es(), formdata(), p(), bits(), ebits(), targmem(), BYREF tptr AS INTEGER, BYREF ptarg AS INTEGER, ltarg())
 ...
 IF bat.targ.hit_dead = NO THEN '---THIS IS NOT DONE FOR ALLY+DEAD------
  FOR j = 0 TO 11
   ...
   IF bslot(j).t(0) = -1 AND bat.acting <> j AND bslot(j).attack > 0 THEN
    ...
    autotarget j, buffer(), bslot(), bstat(), targmem()
   END IF
