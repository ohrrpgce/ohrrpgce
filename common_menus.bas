...
function prompt_from_namelist(title as string, names_list as AnimVariantInfo ptr, helpkey as string, anim_context as AnimationContext, add_none as bool = NO) as string
  ...
  'picker.set_names(names_list, anim_context)
  picker.run()
...
function prompt_variant_name(title as string, anim_context as AnimationContext) as string
  return prompt_from_namelist(title, @builtin_anim_variants(0), "pick_variant_name", anim_context)
end function
